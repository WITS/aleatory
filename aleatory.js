...
window.addEventListener("load", function() {
	...
	if (/\bChrome\b/.test(navigator.userAgent) &&
		...
		recognizer.addEventListener("result", function(event) {
			// console.log(event);
			if (SpeechInfo.awaitingCommand && event.results[event.resultIndex].isFinal) {
				SpeechInfo.lastCommand.transcript = event.results[event.resultIndex][0].transcript.trim();
				SpeechInfo.lastCommand.timeStamp = event.timeStamp;
				console.log(SpeechInfo.lastCommand);
		...
		recognizer.addEventListener("end", function() {
			new Modal({
				from: "bottom",
				text: "<span style='display: inline-block; font-size: 0.5em; line-height: 32px'>speech ended</span>",
				time: 2.5
			});
			listening = false;
		});

		var mic_toggle = document.getElementById("mic-toggle");
		mic_toggle.addEventListener("click", function() {
			if (!listening) {
				recognizer.start();
			} else {
				recognizer.stop();
...
function handle_speech_command() {
	if (current_round.showingResults) {
		if (/next|done|continue/i.test(SpeechInfo.lastCommand.transcript)) {
			current_round.next();
			SpeechInfo.awaitingCommand = false;
		}
		return;
	}

	if (/check/i.test(SpeechInfo.lastCommand.transcript) &&
		player.bid >= current_round.highestBid) {
		player.check();
	}
	if (/match/i.test(SpeechInfo.lastCommand.transcript) &&
		player.bid < current_round.highestBid) {
		player.matchBid();
	}
	if (/(?:raise|res|ra[cz]e|Gr[ae]y's)(?:\s+(\d+))?/i.test(SpeechInfo.lastCommand.transcript)) {
		var x = /\w+\s+(\d+)/i.exec(SpeechInfo.lastCommand.transcript);
		if (x != null) {
			player.raiseBid(+(x[1]));
		} else {
			player.raiseBid(1);
	...
	if (/fold|fooled|full|sold/i.test(SpeechInfo.lastCommand.transcript)) {
		player.fold();
	}
}
...
function Round() {
	...
	this.next_bidder = function() {
		...
		if (this.current_bidder < this.hands.length) {
			...
			if (this.hands[this.current_bidder].money == 0) {
				this.next_bidder();
				return;
			}
			...
			// Check to see if everyone in is at the current bid (or all in)
			for (var x = 0, y = this.hands.length; x < y; x ++) {
				if (!this.hands[x].folded && this.hands[x].bid < this.highestBid && this.hands[x].money > 0) {
					this.current_bidder = -1;
					this.next_bidder();
					return;
				}
...
function display_winners(w) {
	...
	_this2.onclick = current_round.next;
	SpeechInfo.awaitingCommand = true;
